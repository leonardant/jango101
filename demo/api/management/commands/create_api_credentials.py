...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if APIClientCredential.objects.filter(
            user=user
        ).exists():

            self.stdout.write(
                self.style.WARNING(
                    f"API credentials already exist for {username}."
                )
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"API credentials created for {username}"
            )
        )
        ...
        self.stdout.write(
            self.style.WARNING(
                f"Client Secret: {raw_client_secret}"
            )
        )

        self.stdout.write(
            self.style.WARNING(
                "IMPORTANT: Save the client secret now. "
